...
class AdminLists extends BaseAdminDataLists implements ListsExtendInterface, ListsSearchInterface, ListsSortInterface
{
    ...
    public function lists(): array
    {
        ...
        $roleCodeLists = SystemRole::column('role_code', 'id');

        $relationMap = Admin::getAdminServicePersonRelations(array_column($adminLists, 'id'));

        //管理员列表增加角色名称
        foreach ($adminLists as $k => $v) {
            $adminLists[$k]['role_name'] = $roleLists[$v['role_id']] ?? '';
            $adminLists[$k]['role_code'] = $roleCodeLists[$v['role_id']] ?? '';
            if ($v['root'] == 1) {
                $adminLists[$k]['role_name'] = '系统管理员';
            }
            $adminLists[$k]['dept_name'] = empty($v['dept_name']) ? '-' : $v['dept_name'];

            $adminLists[$k]['service_person_relations'] = $relationMap[$v['id']] ?? [];
        }
    ...
    private function getAdminServicePersonRelations ($adminIds = [])
    {
